...
import { updateFamilyCase } from "@/lib/dashboard/family-case-actions";
import {
...
type EditFamilyCaseFormProps = {
  familyCase: FamilyCase;
};

export function EditFamilyCaseForm({ familyCase }: EditFamilyCaseFormProps) {
  const updateAction = updateFamilyCase.bind(null, familyCase.id);
  const [state, formAction] = useActionState(updateAction, initialState);

  return (
    <form action={formAction} className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
      <div>
        <h2 className="text-2xl font-semibold text-stone-950">Edit family case details</h2>
        <p className="mt-3 text-sm leading-6 text-stone-700">
          Keep this simple profile current so the dashboard can organise the next steps around
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <label className="block md:col-span-2" htmlFor="care_recipient_name">
          ...
          <input
            ...
            type="text"
            autoComplete="off"
            defaultValue={familyCase.care_recipient_name}
        ...
        <label className="block" htmlFor="relationship_to_user">
          ...
          <select
            ...
            defaultValue={familyCase.relationship_to_user}
        ...
        <label className="block" htmlFor="care_recipient_state">
          ...
          <select
            ...
            defaultValue={familyCase.care_recipient_state ?? ""}
            className={selectClass}
        ...
        <label className="block md:col-span-2" htmlFor="care_recipient_suburb">
          ...
          <input
            ...
            defaultValue={familyCase.care_recipient_suburb ?? ""}
            className={inputClass}
        ...
        <label className="block" htmlFor="current_living_situation">
          ...
          <select
            ...
            defaultValue={familyCase.current_living_situation}
            className={selectClass}
        ...
        <label className="block" htmlFor="current_status">
          ...
          <select
            id="current_status"
            name="current_status"
            defaultValue={familyCase.current_status}
            className={selectClass}
...
function SubmitButton() {
  ...
  return (
    ...
    >
      {pending ? "Saving family case..." : "Save family case"}
    </button>
  );
}
