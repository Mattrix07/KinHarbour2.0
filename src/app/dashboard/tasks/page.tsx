...
export default async function DashboardTasksPage() {
  ...
  if (!familyCase) {
    return (
      <EmptyTasksPage
        title="Create a family case before adding tasks."
        description={
          errorMessage ??
          "KinHarbour needs a family case so tasks can be attached to the aged care journey you are organising."
        }
        ctaHref="/dashboard"
  ...
  const [{ members, errorMessage: membersError }, { tasks, errorMessage: tasksError }] =
    await Promise.all([
      getFamilyMembersForCase(familyCase.id),
      getTasksForFamilyCase(familyCase.id),
    ]);
    ...
    <main className="mx-auto w-full max-w-7xl px-5 py-10 sm:px-6 lg:px-8">
      <section className="rounded-lg border border-stone-200 bg-white p-6 shadow-sm sm:p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-3xl">
            <p className="text-sm font-semibold uppercase text-[#146c60]">Family tasks</p>
            <h1 className="mt-3 text-4xl font-semibold leading-tight text-stone-950 sm:text-5xl">
              Shared tasks for {familyCase.care_recipient_name}
            </h1>
            <p className="mt-4 text-lg leading-8 text-stone-700">
              Track practical follow-ups across the people helping with aged care decisions.
            </p>
          </div>
      ...
      <div className="mt-8">
        <DisclaimerBox title="Coordination only">
          <p>
            Tasks help the family coordinate follow-ups. They do not replace My Aged Care,
            provider advice, medical advice, legal advice, or financial advice.
          </p>
        </DisclaimerBox>
      </div>
